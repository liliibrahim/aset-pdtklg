...
<div class="px-6 py-6">
    ...
    {{-- Statistik ringkas pengguna sistem --}}
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">

        {{-- Jumlah keseluruhan pengguna --}}
    ...
    {{-- Ringkasan konfigurasi struktur organisasi --}}
    ...
    {{-- Paparan aktiviti terkini sistem --}}
    <div class="bg-white shadow rounded-xl overflow-hidden">
        ...
        <table class="w-full border text-sm">
            ...
            <tbody>
                {{-- Senarai log aktiviti terkini --}}
                @foreach($aktivitiTerkini as $log)
                <tr>
                    <td class="border px-3 py-2">{{ $log->created_at->format('d/m/Y H:i') }}</td>
                    <td class="border px-3 py-2">{{ $log->user->name ?? '-' }}</td>
                    <td class="border px-3 py-2">{{ $log->aktiviti }}</td>
                    <td class="border px-3 py-2">{{ $log->tindakan }} </td>
                    <td class="border px-3 py-2">{{ $log->modul }}</td>
                    <td class="border px-3 py-2">{{ $log->aset ?? '-' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
